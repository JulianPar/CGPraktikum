...
void MyGLWidget::recursive_render(const aiScene *scene,const aiNode* node,QMatrix4x4 M) {
    ...
    for (unsigned int n = 0;n < node->mNumMeshes; n++) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, this->vboSolidSphereId);
        ...
        glDrawArrays(GL_TRIANGLES,0,this->vboSolidSphereId);
    }

    for (unsigned int n = 0; n < node->mNumChildren; n++)
        recursive_render(scene, node->mChildren[n],M);
}
